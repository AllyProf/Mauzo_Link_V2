    ...
    <div class="tile p-3">
      <div class="row align-items-center">
        <div class="col-md-6 border-right">
          <form method="GET" action="{{ route('Manager.daily-master-sheet') }}" class="form-inline">
            ...
            <a href="{{ route('Manager.daily-master-sheet.history') }}" class="btn btn-secondary btn-sm"><i class="fa fa-history"></i> Full History</a>
          </form>
        </div>
      ...
      <div class="row">
        ...
        <div class="col-md-7 pl-md-5">
          @if($ledger->status === 'open')
            ...
            <form id="closingForm" action="{{ route('Manager.daily-master-sheet.close') }}" method="POST">
              @csrf
          ...
          @else
            ...
            <div class="tile p-3 bg-light border">
               ...
               <hr>
               <div class="text-center small text-muted">Closed by <strong>{{ $ledger->Manager->name ?? 'System' }}</strong> at {{ $ledger->closed_at?->format('h:i A') ?? '—' }}</div>
...
$(document).ready(function() {
  $('#expenseForm').on('submit', function(e) {
    ...
    $.ajax({
      url: "{{ route('Manager.daily-master-sheet.expense') }}",
      method: "POST",
      data: $(this).serialize(),
  ...
  $('.delete-expense-btn').click(function() {
      ...
      Swal.fire({
          ...
          if(result.isConfirmed) {
              $.post("{{ url('Manager/daily-master-sheet/expense') }}/" + id + "/delete", { _token: "{{ csrf_token() }}" }, function(res) {
                  if(res.success) {
                      location.reload();
                  } else {
